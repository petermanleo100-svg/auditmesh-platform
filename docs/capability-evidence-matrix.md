...
| Release-bound environment evidence | schema-v2 fail-closed manifest binds source reconciliation, control approval, IdP, PITR, rollback and alert-delivery evidence to the exact release SHA with independent verifier, UTC freshness, durable URI and SHA-256 digest; positive and bypass-negative tests plus non-zero CLI contract |
