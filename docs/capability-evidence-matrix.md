...
| Release-bound environment evidence | schema-v3 fail-closed bundle binds source reconciliation, control approval, IdP, PITR, rollback and alert-delivery files to the exact release SHA with independent verifier, UTC freshness, durable URI and recomputed SHA-256; positive and tamper/bypass-negative tests plus non-zero CLI contract |
