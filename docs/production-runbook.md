...
Use `auditmesh-operations` for backup, restore and evidence verification. Set `AUDITMESH_TEXTFILE_DIR` to a Node Exporter textfile-collector directory (or equivalent). Every command atomically writes a separate bounded-label metric and preserves last-success time after failure. Schedule `backup-create`, `evidence-verify`, and `business-status-export`; run the last command at least every five minutes with a separate audited operations database identity that can aggregate all tenants. The business exporter atomically emits only low-cardinality aggregate gauges for overdue critical cases, current source failures and source heartbeat gaps. Page on non-zero exit, `operation_success == 0`, `node_textfile_scrape_error{job="auditmesh-node-exporter"} == 1`, business exporter age beyond ten minutes, or backup age beyond the 15-minute RPO. An unwritable or unreadable metrics directory is a deployment failure. The API rejects bodies over 2 MiB and bounds identity/control fields.

Register each connector through `PUT /sources/{source_id}` with an administrator identity, binding it to the collector token `sub` and an approved 60-second-to-seven-day maximum silence window. The bound collector reports each successful or failed cycle to `POST /sources/{source_id}/result`; failed reports require a bounded error code and successful reports clear the current-failure state. A collector whose `sub` does not match the source contract receives `403`. Source IDs and tenants are never exported as Prometheus labels.

Load `deploy/prometheus/auditmesh-alerts.yml` into the approved Prometheus-compatible backend and map `owner=platform-operations`, `owner=control-operations`, and `owner=integration-operations` plus severity to named receivers. CI validates syntax and executes synthetic firing scenarios for readiness, 5xx rate, p95 latency, scheduled-operation/exporter failure, stale backup, critical overdue cases, current source failures and source gaps with `promtool`; receiver credentials and a real test notification remain environment acceptance evidence. CI runs `pip-audit`, retains an SPDX JSON image SBOM for 30 days, and blocks image vulnerabilities that are High/Critical with a known fix. Unfixed findings require explicit release risk review.
