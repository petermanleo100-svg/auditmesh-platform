...
def main():
 ...
 if args.command=="preflight":
  ...
  with db.connect(args.tenant) as conn:result=verify_evidence(conn,args.tenant)
 print(json.dumps(result,ensure_ascii=False,sort_keys=True));raise SystemExit(0 if result.get("valid",True) else 2)
