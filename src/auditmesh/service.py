...
class AuditMeshService:
 ...
 def ingest(self,event:dict):
  ...
  with self.db.connect(self.tenant_id) as conn:
   ...
   conn.execute(insert(ControlEvent).values(tenant_id=self.tenant_id,event_id=event["event_id"],event_type=event["event_type"],actor=event["actor"],resource=event["resource"],occurred_at=event["occurred_at"],payload_json=canonical(event.get("payload",{})),raw_event_json=canonical(event),evidence_hash=evidence))
 ...
 def cases_for_event(self,event_id):
  with self.db.connect(self.tenant_id) as conn: return [dict(r) for r in conn.execute(select(AuditCase).where(AuditCase.tenant_id==self.tenant_id,AuditCase.event_id==event_id)).mappings()]
