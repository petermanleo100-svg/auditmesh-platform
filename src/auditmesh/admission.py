...
REQUIRED_CONTROLS={"alert_delivery","control_owner_approval","managed_postgres_pitr_restore","oidc_key_rotation","source_connector_reconciliation","staging_migration_rollback"};_SHA=re.compile(r"^[0-9a-f]{40}$")
def verify_admission(path:str|Path,release_sha:str,max_age_hours:int=168)->dict:
 if not _SHA.fullmatch(release_sha):return {"valid":False,"errors":["release_sha must be a lowercase 40-character Git SHA"]}
