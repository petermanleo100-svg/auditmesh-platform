...
from auditmesh.models import SourceContract
...
def test_encrypted_backup_clean_restore_and_evidence_verification(tmp_path):
 source=Database(tmp_path/"source.db");source.initialize();service=AuditMeshService(source,"alpha");service.install_policies();case=service.ingest(event())[0];service.transition(case["case_id"],"auditor","INVESTIGATING","triage");service.register_source("erp","collector-alpha",300)
 ...
 with target.connect() as conn:assert verify_evidence(conn)["valid"] and conn.execute(select(SourceContract.source_id)).scalar_one()=="erp"
